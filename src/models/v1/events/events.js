...
const eventsModel = {
  list: (data, params, callback) => {
    ...
    if (params.orderBy) {
      ...
      if (params.orderType) {
        Object.assign(data, {
          orderBy: { attribute: params.orderBy, type: params.orderType }
        });
      } else {
  ...
  illegalTimeFrameChecker: (event, callback) => {
    ...
    database.query(newState, (err, events) => {
      ...
      console.log(events);
      if (events.hasOwnProperty("rowCount")) {
        if (events.rowCount > 0)
          return callback("Illegal time frame, overlapping events");
      } else {
        callback("Hrmm there is an error somewhere...");
      }
